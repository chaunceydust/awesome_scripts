...
#		c0['class_id']=cutline[5]
		items=items+[c0]
################ 抓取产品信息	
	out0 = open('Parse_Shop.面部护肤.txt','w')
	print('Shop_ID'+'\t'+'Shop_Name'+'\t'+'URL1'+'\t'+'URL2'+'\t'+'shopID'+'\t'+'venderID',file=out0)
	out0.close()	
	out0 = open('Parse_Shop.面部护肤.txt','a')
################################
	out1 = open('Parse_Product.面部护肤.txt','w')
	...
	out1.close()	
	out1 = open('Parse_Product.面部护肤.txt','a')
####################################
	out2 = open('Parse_Comment.面部护肤.txt','w')
	print('Name'+'\t'+'Count'+'\t'+'Category'+'\t'+'Class'+'\t'+'ProductID',file=out2)
	out2.close()	
	out2 = open('Parse_Comment.面部护肤.txt','a')
	for i in range(2884,len(items)):
		n1=	items[i]['category']	
